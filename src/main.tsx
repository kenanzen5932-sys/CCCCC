...
function Complete() {
  const [viewed,setViewed]=useState(0);const [bonusOpen,setBonusOpen]=useState(false);const [bonusUsed,setBonusUsed]=useState(false);const [bonusCount,setBonusCount]=useState(5);const bonusUnlocked=viewed>=2;
  const openBonus=()=>{if(bonusUsed||bonusOpen)return;setBonusCount(5);setBonusOpen(true)};
  useEffect(()=>{if(!bonusOpen)return;const timer=setInterval(()=>setBonusCount(count=>{if(count<=1){clearInterval(timer);setBonusOpen(false);setBonusUsed(true);return 0}return count-1}),1000);return()=>clearInterval(timer)},[bonusOpen]);
  return <motion.main className="complete-page page" initial={{opacity:0}} animate={{opacity:1}}><motion.div className="complete-emblem" initial={{scale:0,rotate:-90}} animate={{scale:1,rotate:0}} transition={{type:'spring',duration:1}}><Check size={42}/><i/></motion.div><p className="mono green">// TÜM PUZZLELAR TAMAMLANDI</p><h1>NEVER<br/><span>BULUNDU</span></h1><p className="complete-copy"><strong>Tebrikler Eliza.</strong><br/>Sınavın bütün aşamalarını başarıyla tamamladın.</p><div className="unlocked"><span/><p>2 GİZLİ DOSYANIN KİLİDİ AÇILDI</p><span/></div><div className="image-grid"><SecureImage index={1} onViewed={()=>setViewed(v=>v+1)}/><SecureImage index={2} onViewed={()=>setViewed(v=>v+1)}/></div>{viewed>0&&<motion.div className="viewed-warning" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}><CircleAlert size={15}/> Bu görsel artık görüntülenemez.</motion.div>}{bonusUnlocked&&<motion.section className={`bonus-unlocked ${bonusUsed?'used':''}`} initial={{opacity:0,y:24,scale:.97}} animate={{opacity:1,y:0,scale:1}} transition={{type:'spring',duration:.65}}><div className="bonus-title">{bonusUsed?<X size={20}/>:<ShieldCheck size={20}/>}<div><small>// EK ÖDÜL DOSYASI</small><strong>{bonusUsed?'BONUS FOTOĞRAF KİLİTLENDİ':'BONUS FOTOĞRAF KİLİDİ AÇILDI'}</strong></div></div><button className="bonus-photo-button" onClick={openBonus} disabled={bonusUsed||bonusOpen}><div className="bonus-classified-art"><img src="/bonus-photo.png" alt="" draggable={false}/><div className="watermark">NEVER // BONUS</div></div><div className="bonus-thumb-overlay">{bonusUsed?<><X/><span>GÖRSEL KİLİTLENDİ</span></>:<><Eye/><span>TEK SEFER GÖRÜNTÜLE</span></>}</div><span className="bonus-image-label">BONUS_01.ENC</span></button></motion.section>}<AnimatePresence>{bonusOpen&&<motion.div className="bonus-modal" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0,filter:'blur(24px)'}}><img src="/bonus-photo.png" alt="Bonus fotoğraf" draggable={false}/><div className="countdown"><Timer/><strong>{bonusCount}</strong><span>SANİYE SONRA KAPANACAK</span></div></motion.div>}</AnimatePresence></motion.main>
}
