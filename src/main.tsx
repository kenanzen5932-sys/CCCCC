...
function Complete() {
  const [viewed,setViewed]=useState(0);const [bonusOpen,setBonusOpen]=useState(false);const bonusUnlocked=viewed>=2;
  return <motion.main className="complete-page page" initial={{opacity:0}} animate={{opacity:1}}><motion.div className="complete-emblem" initial={{scale:0,rotate:-90}} animate={{scale:1,rotate:0}} transition={{type:'spring',duration:1}}><Check size={42}/><i/></motion.div><p className="mono green">// TÜM PUZZLELAR TAMAMLANDI</p><h1>NEVER<br/><span>BULUNDU</span></h1><p className="complete-copy"><strong>Tebrikler Eliza.</strong><br/>Sınavın bütün aşamalarını başarıyla tamamladın.</p><div className="unlocked"><span/><p>2 GİZLİ DOSYANIN KİLİDİ AÇILDI</p><span/></div><div className="image-grid"><SecureImage index={1} onViewed={()=>setViewed(v=>v+1)}/><SecureImage index={2} onViewed={()=>setViewed(v=>v+1)}/></div>{viewed>0&&<motion.div className="viewed-warning" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}><CircleAlert size={15}/> Bu görsel artık görüntülenemez.</motion.div>}{bonusUnlocked&&<motion.section className="bonus-unlocked" initial={{opacity:0,y:24,scale:.97}} animate={{opacity:1,y:0,scale:1}} transition={{type:'spring',duration:.65}}><div className="bonus-title"><ShieldCheck size={20}/><div><small>// EK ÖDÜL DOSYASI</small><strong>BONUS FOTOĞRAF KİLİDİ AÇILDI</strong></div></div><button className="bonus-photo-button" onClick={()=>setBonusOpen(true)}><img src="/bonus-photo.png" alt="Bonus fotoğraf önizlemesi" draggable={false}/><span><Eye size={18}/> BONUS FOTOĞRAFI AÇ</span></button></motion.section>}<AnimatePresence>{bonusOpen&&<motion.div className="bonus-modal" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setBonusOpen(false)}><button aria-label="Bonus fotoğrafı kapat" onClick={()=>setBonusOpen(false)}><X/></button><img src="/bonus-photo.png" alt="Bonus fotoğraf" draggable={false}/><p>DOSYAYI KAPATMAK İÇİN EKRANA DOKUN</p></motion.div>}</AnimatePresence></motion.main>
}
